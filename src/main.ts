...
class VirtualMic {
  ...
  async start() {
    const virtualMicsPath = `${Deno.env.get("XDG_RUNTIME_DIR")}/virtual_mics`;
    await Deno.mkdir(virtualMicsPath, { recursive: true });

    const virtualMicPipePath = `${virtualMicsPath}/virtual_mic_pipe${this.#id}`;
